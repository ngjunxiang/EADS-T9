...
def load_input(filepath):
    headers = ['job-id', 'userid', 'expected-time-to-completion',
...
def init_simulation(input_df, output):
    ...
    while (len(jobs_df.index) > 0):
        ...
        current_job_queue_df = current_job_queue_df.sort_values(
            by=['expected-time-to-completion'])
        ...
        if len(current_job_queue_df) > 0:
                    ...
                    for gpu_id in range(num_gpus):
                        ...
                        if gpus[gpu_id]['memory-taken'] + job[1]['memory'] <= gpu_fixed_memory and ongoing_jobs_in_current_gpu < 2:
                            if gpus[gpu_id]['number-of-current-jobs'] == 0:
                                ...
                                shortest_completion_time = current_time + \
            ...
            # allocate 1 gpu for long jobs
                ...
                if gpu_to_assign != -1:
                    ...
                    # series to append
                    append_series = pd.Series([job[1]['job-id'], job[1]['userid'], job[1]['expected-time-to-completion'], job[1]['memory'], job[1]['priority'],
                                               job[1]['arrival-time'], current_time, gpu_to_assign, shortest_completion_time], index=output.columns)
                    # print(append_series)
                    ...
                    # print(output)
                    current_job_queue_df = current_job_queue_df[current_job_queue_df['job-id']
                                                                != job[1]['job-id']]
                    # print('job id: ' + str(job[1]['jobid']))
                    jobs_df = jobs_df[jobs_df['job-id'] != job[1]['job-id']]
...
def calculate_consecutive_completion_time(ongoing_jobs, current_time, gpu_id, gpu_details, job):
    completed_time = 0
    ongoing_jobs_in_current_gpu = ongoing_jobs[ongoing_jobs['gpuid'] == gpu_id]
    ongoing_jobs_in_current_gpu = ongoing_jobs_in_current_gpu[
        ongoing_jobs_in_current_gpu['completion-time'] >= current_time]
...
def calculate_concurrent_completion_time(ongoing_jobs, current_time, gpu_id, gpu_details, job):
    ...
    ongoing_jobs_in_current_gpu = ongoing_jobs_in_current_gpu[
        ongoing_jobs_in_current_gpu['completion-time'] >= current_time]
    ongoing_jobs_in_current_gpu = ongoing_jobs_in_current_gpu.sort_values(by=[
                                                                          'completion-time'])
    ...
    while job_not_completed:
        ...
        if len(ongoing_jobs_in_current_gpu) > 0:
            ...
            new_additional_time = math.ceil(
                time_left_for_curr_k_value / previous_k_penalty * k + (job[1]['memory']))

            # new end time for current job
            new_shortest_job_end_time = shortest_job_end_time - \
                time_left_for_curr_k_value + new_additional_time + leftover_time
            ...
            for index in range(len(indexes)):
                # print(index)
                curr_time_taken = ongoing_jobs_in_current_gpu[
                    'completion-time'].iloc[index - num_dropped_indexes]
                # print(curr_time_taken)
                new_time_taken = curr_time_taken - \
                    time_left_for_curr_k_value + new_additional_time + 1
                ongoing_jobs_in_current_gpu.at[indexes[index],
                                               'completion-time'] = new_time_taken
                # print(new_time_taken)
                # print(new_shortest_job_end_time)

                if new_time_taken == new_shortest_job_end_time:
                    new_completion_times.append(
                        (ongoing_jobs_in_current_gpu.iloc[index - num_dropped_indexes]['job-id'], new_shortest_job_end_time))
                    # print('ongoing_jobs_in_current_gpu: ', ongoing_jobs_in_current_gpu)
                    ongoing_jobs_in_current_gpu = ongoing_jobs_in_current_gpu.drop(
                        indexes[index])
            ...
            job_completion_time = job_completion_time - \
                time_left_for_curr_k_value + new_additional_time
...
def clean_up(output):
    output['gpuid'] = output['gpuid'].apply(lambda x: x + 1)
    del output['completion-time']
    return output

# containers
output = pd.DataFrame(columns=['job-id', 'userid', 'expected-time-to-completion', 'memory',
                               'priority', 'arrival-time', 'submitted', 'gpuid', 'completion-time'])  # result - to be written into csv
gpus = {}
